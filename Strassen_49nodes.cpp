...
int main (int argc, char *argv[]) { 
    ...
    if (rank == 0) {
        ...
        // Splitting every matrix that needs to be multiplied together into 14 pieces
        // Turns 14 pieces into 98 pieces
        vector<vector<vector<int>>> s1, s2, s3, s4, s5, s6, s7;

        vector<vector<vector<vector<int>>>> split;

        s1 = sevenSplit(split1[0],split1[1]);
        s2 = sevenSplit(split1[2],split1[3]);
        s3 = sevenSplit(split1[4],split1[5]);
        s4 = sevenSplit(split1[6],split1[7]);
        s5 = sevenSplit(split1[8],split1[9]);
        s6 = sevenSplit(split1[10],split1[11]);
        s7 = sevenSplit(split1[12],split1[13]);

        split = {s1, s2, s3, s4, s5, s6, s7};
        ...
        for (int a = 0; a < split.size(); a++) { 
            for (int b = 0; b < split[0].size(); b++) { 
                for (int c = 0; c < split[0][0].size(); c++) { 
                    for (int d = 0; d < split[0][0][0].size(); d++) { 
                        scatterArray.push_back(split[a][b][c][d]);
                    }
                }
            }
        }
    ...
    // Create the gather array, need to turn the matrix into vector format for gather
    ...
    if (rank == 0) {
        ...
        // Combining every 7 matrixes from nodes 0-6, 7-13, etc. 

        vector<vector<vector<int>>> firstCombine;
        vector<vector<int>> result;
        for (int i = 0; i < matrixSep.size(); i+=7) { 
            result = combine({matrixSep[i], matrixSep[i+1], matrixSep[i+2], matrixSep[i+3], matrixSep[i+4], matrixSep[i+5], matrixSep[i+6]});
            firstCombine.push_back(result);
            result.clear();
        }

        // Combining the final 7 matrixes into 1
        for (int i = 0; i < firstCombine.size(); i+=7) { 
            result = combine({firstCombine[i], firstCombine[i+1], firstCombine[i+2], firstCombine[i+3], firstCombine[i+4], firstCombine[i+5], firstCombine[i+6]});
        }
